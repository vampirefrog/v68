...
04:3#     14:R      24:J      34:_      44:8      54:HELP   64:F2
...
Ret	キーデータ
	...
	SHIFT、CTRL、OPT.1、OPT.2 については、離した時にもスキャンコードに $80
	を足した値を返す.
	ASCII コードは SHIFT、CTRL、かな、CAPS により変化し、また ASCII 文字が
	定義されていない場合は 0 になる.
	通常テレビコントロールに割り当てられているキーは入力されないが、押し下
	げ状態には反映される.
...
Arg	d1.b	キーリピート間隔(0〜15)

	キーリピートの間隔を (30＋5×d1.b2)ms に設定する.
...
Ret	d1.w = -1 の時、現在のモード(0〜27)

	CRT モードを設定する. テキスト画面のプレーン 0/1 をクリアして表示モー
	ドに、グラフィック画面とスプライト画面はクリアせずに無表示モードにする.
	テキストパレットは初期化される.
	d1.w = 19 の 640 x 480 のモードは ROM IOCS version 1.2 以上で使用可能.
	d1.w = 20〜27 は表示画面サイズより小さい実画面のグラフィックを表示する
	モードで、ROM IOCS version 1.3 だけで使用可能. なお、ハードウェア的に
	は ROM IOCS version 1.1 以下(XVI まで)なら d1.w = 23 及び 27 以外が、
	ROM IOCS version 1.2(Compact)なら全てのモードが実現可能である. これら
	はそのバージョンの ROM IOCS に用意されていないから指定できないだけなの
	で、自前で I/O を設定すれば表示できる.
...
Ret	ステータス(d2.l = -1 の時はカラーコード)
	...
	IOCS _TPALET($13) とは違い、パレットごとに独立して設定出来る.

==============================================================================

$15	_TCOLOR		テキストプレーン指定

Arg	d1.b	アクセスプレーンの指定
		d1.b =	%???1	プレーン 0
			%??10	〃	 1
			%?100	〃	 2
			%1000	〃	 3
...
Arg	d1.w	バッファの X 方向のバイト数-1
	d2.w	〃	   Y	〃 ドット数-1
	d3.l	(VRAM の X 方向-バッファの X 方向)のバイト数
	a1.l	バッファのアドレス
	a2.l	VRAM	〃

Ret	d0/d1/d2/a1/a2 は内容が保証されない.
...
Ret	Cursor position after display
	d0.hw	column
	d0.w	row

	Display characters
	漢字を表示する場合、第 1/第 2 バイトを 2 回に分けてもかまわない.
	画面の右端で半角 1 文字しか表示出来ない時に全角文字を表示しようとした場合、半角スペースを表示して改行してから全角文字を表示する.
...
Ret	d0.b	変更前の表示属性(負数なら表示属性の指定が異常)
...
Arg	d1.w	桁位置(ただし、d1.w = -1 の時カーソル位置の収得のみ)
	d2.w	行〃

Ret	移動前のカーソル位置(-1 ならカーソル位置の指定が異常)
	d0.hw	桁位置
	d0.w	行〃
...
Ret	d0.l =	0	正常終了
	...
	異常終了した時のRetは通常 -1 だが、ROM IOCS version 1.3 未満でかつカ
	ーソル位置が原因の場合はそれ以外の負数になることがある.

==============================================================================

$2c	_B_INS		複数行挿入

Arg	d1.b	挿入行数(0 の時は 1 と見なします)

Ret	常に 0

	カーソル位置に指定した数だけ空行を挿入する. カーソル以下の行はスクロー
	ルする. 挿入後、カーソルは左端に移動する.
	ROM IOCS version 1.3 未満ではArgの d1.w の上位バイトを 0 にしておかな
	いと正常に動作しないので注意すること.
...
Arg	d1.b	削除行数(0 の時は 1 と見なします)

Ret	常に 0

	カーソル位置から指定した行数だけ削除する. カーソル行以下の行はスクロー
	ルする. 削除後、カーソルは左端に移動する.
	ROM IOCS version 1.3 未満ではArgの d1.w の上位バイトを 0 にしておかな
	いと正常に動作しないので注意すること.
...
Ret	d0.l	常に 0
	...
	範囲内に影響する. ただし、スクロール/画面クリアは表示範囲外の桁にも影
	響が出る. 表示範囲だけスクロールさせるには IOCS _B_CONMOD($ad) のモー
	ド 18:ソフトコピースクロールを指定する.
	IOCS.X 組み込み時はArgの d1.hw に 8 の倍数を指定出来るが、ROM IOCS
	version 1.3 未満で 8 の倍数を指定するとアドレスエラーが発生するので注
	意すること.
...
Ret	変更前のアドレス(ROM IOCS version 1.3 のみ)
	...
	レスを設定する. 処理アドレスに 0 を指定すると標準アドレスが設定される.
	設定したアドレスが呼び出される時は、d0.w に ESC[> の次の 2 バイトデー
	タ、a0.l に ESC[> のアドレスがセットされている.
...
Arg	d1.l	フォントアドレス
	d2.hw	最上位ビットが 1 でアドレスの収得のみ
	d2.w	外字グループ番号
		d2.w =	0	16x16ドット	JIS:$2c21〜$2d7e
			1	16x16  〃	   :$7621〜$777e
			2	 8x16  〃	   :$f400〜$f5ff
			3	24x24  〃	   :$2c21〜$2d7e
			4	24x24  〃	   :$7621〜$777e
			...
			14	16x16  〃(第１水準文字)
			15	16x16  〃(第２水準文字)
			0x80	拡張外字処理アドレスへのポインタを得る

Ret	変更前のアドレス
	アドレス設定時に -1 が返った時はエラー(外字グループ番号がおかしいか、
	奇数アドレスを指定した場合).

	外字フォントデータのアドレスを設定し、以前の設定アドレスを返す. d2.l
	の最上位ビットを 1 にした場合(負数にした場合)は設定を変更せずに、現在
	の外字フォントデータのアドレスを収得する.
	HIOCS.X 組み込み時には d2.w が 6 以上で普通の文字のフォントも設定可能.
	ROM IOCS では奇数アドレスを設定してもエラーにならないので注意すること.

	d2.w = 0x80 は拡張外字処理を行うアドレスを保持する 1 ロングワードのワ
	ークへのポインタを返す(最上位ビットを 1 にする必要はない).
...
Ret	d0.l =	0	出力不可能
	...
	Argの d1.w は IOCS _OUTPRN($3f) 用のプリントフォーマット.
...
Ret	SASI-HD	: ステータス
	...
	ディスクの指定位置までシークする.
	SASI-HD のシークでRetが $ffffff?? の場合、$?? はセンスバイトの先頭バ
	イトを示す.
...
Ret	常に 0
	...
	ので、IOCS _DATEBCD($50) でバイナリの日付を BCD に変換した後にこのコー
	ルで設定する.
...
Arg	a1.l	時刻を表す文字列のアドレス(区切りは ':' のみ)
		形式は 'hh:mm:ss',0
...
Arg	d1.w	Wait mode / frequency / output mode
		bit     15	Wait mode (0: normal 1: no wait)
		bit 10〜 8	Sampling frequency (0:3.9kHz 1:5.2kHz 2:7.8kHz 3:10.4kHz 4:15.6kHz)
		bit  1〜 0	Output mode (0: None 1: Left 2: Right 3: Right and left)
	d2.l	Number of bytes of playback data
	a1.l	〃	    Address

Ret	The content is not guaranteed for d0/d2/a1.

	Play ADPCM.
	If the data is less than $ff00 bytes, it returns immediately.
	If the DMA is unusable, wait until it becomes available.
...
Arg	d1.b	Control contents (0: end 1: interruption 2: resume)

Ret	d0.l =	0 Success
           -1 Control mode is invalid

	Control the execution of ADPCM.
	Please note that in ROM IOCS, calling with shortcut will return
	with interrupt disabled status.
...
Ret	d0.l =	0	割り込みを設定/禁止した
	...
	実際には Timer-A に割り込みを設定している. ROM IOCS では MFP への設定
	方法が間違っている為に、電源投入後または以前に大きいカウンタ値で設定し
	ていた時は、最初の割り込みが発生するタイミングが遅れる.

	IOCS _CRTMOD の画面モード別のおおよその秒間表示回数(実測値)を以下に示
	す. モード 19 以降は未計測.
...
Arg	d1.w	カーソル番号(0〜15)
	...
	マウスカーソルの大きさは 16 x 16 ドットで、1 ドットが 1 ビットに対応す
	る. パターンデータの内容は以下の通り(カラーコード 8〜15 は 4〜7 より優
	先される).
...
Ret	d0.l	Hour of elapsed time (0〜23:59:59"99=8639999, 1/100 unit)
	d1.l	Days of elapsed time (0〜65535)

	Check the time since the ROM started up.
	ROM IOCS では、ショートカットで呼び出すと割り込み禁止状態になったままで帰るので注意すること.
...
Ret	d0.l	ユーザモードに切り換えた場合は常に 0 を返す.
...
Arg	d1.l	Number of transferred bytes - 1
	a1.l	Source address
	a2.l	Destination address

Ret	a1.l	Next address
	a2.l	〃
	d1.l Is destroyed.

	Transfer multi-byte data from specified address.
	Transfer direction is different from IOCS _B_MEMSET (*a2++ = *a1++).
...
Arg	d1.w	アクセスページ(0〜3)
	...
	IOCS _APAGE($b1) とほぼ同じであるが、存在しないページを指定してもエラ
	ーにならないので注意すること.
...
Arg	d1.w	X 座標
	...
	グラフィック画面にビットパターンを書き込む.
	ビットが 1 のドットは IOCS $95 で設定したカラーコードが書き込まれ、0
	のドットはバックカラーコードが書き込まれる.
...
$ae	_OS_CURON	カーソル表示
	...
	カーソル表示をする. コンソールに対して ESC[>5l を出力した時に使用され
	る. IOCS $20〜$2f に対して有効.
...
$af	_OS_CUROF	カーソル消去
	...
	カーソル表示しない. コンソールに対して ESC[>5h を出力した時に使用され
	る. IOCS $20〜$2f に対して有効.
...
Arg	d1.b	表示位置を設定するページ(bit 0〜3 がページ 0〜3 に対応
					 0 で全てのページ)
	d2.w	X 座標
	d3.w	Y 〃
